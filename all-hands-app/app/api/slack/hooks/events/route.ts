    ...
    switch (event.type) {
        ...
        case 'message':
            if (event.channel_type === 'im' && !!event.client_msg_id) {
                ...
                // waitUntil ensures background processing completes even after response
                waitUntil(
...
/**
 * Retrieves a specific session by month_year with all its questions and answers
 * @param monthYear - The month_year to search for (e.g., "January 2025")
 * @returns SessionWithDetails object or null if not found
 */
async function getSessionWithDetailsByMonthYear(monthYear: string): Promise<SessionWithDetails | null> {
    try {
        const supabase = createServerSupabaseClient();

        // Get the specific session by month_year (since it's unique)
        ...
        // Get questions for this specific session
        ...
        if (questionsError) {
            console.error('Error fetching questions for session:', questionsError);
            return null;
        }

        let questionsWithAnswer: (Question & { answer?: Answer })[] = questions || [];

        // Only include answers if session is completed
        if (session.status === 'completed' && questions && questions.length > 0) {
            // Get answers for the questions in this session
            ...
            if (answersError) {
                console.error('Error fetching answers for session:', answersError);
                // Still return session with questions, just without answers
            } else {
                // Map questions with their answers
                questionsWithAnswer = questions.map(q => ({
                    ...q,
        ...
        console.error('Error in getSessionWithDetailsByMonthYear:', error);
        return null;
    }
}
        ...
        switch (messageType.request_type) {
            ...
            case "get_session_records":
                try {
                    if (messageType.month_year) {
                        await slack.chat.postMessage({
                            channel: event.channel!,
                            text: "Searching for session records...",
                        });
                        // Get session with details directly from database
                        const session = await getSessionWithDetailsByMonthYear(messageType.month_year);
            ...
            case "ask_anything":
                try {
                    ...
                    const aiData = await AskAnythingService.allHands(event.text || "");
